...
/* ==================================================================================
   1. DESIGN SYSTEM & TOKENS (Addressing Point 7)
   Centralized source of truth for colors and styles.
   ================================================================================== */
...
/* ==================================================================================
   2. DATA LAYER (CMS)
   Separated from logic. This would live in /content/siteContent.ts in production.
   ================================================================================== */
...
const SITE_CONTENT = {
  ...
  thinking: {
    ...
    posts: [
      ...
      // ... (other posts omitted for brevity but would be here)
    ]
  },
  prvaah: {
    title: "prvaah",
...
/* ACCESSIBILITY FIX: 
  SoftCard is now polymorphic. Renders a <button> if interactive, otherwise a <div>.
  Addresses "Clickable divs instead of buttons" (Point 4)
*/
...
/* ==================================================================================
   ...
   Logically separated to address "Doing too much" (Point 1)
   wrapped in memo for Performance (Point 3)
   ================================================================================== */
    ...
    return (
        <div className="w-full max-w-4xl mx-auto animate-fade">
            <div className="mb-8 grid md:grid-cols-2 gap-8 items-center">
                ...
                {/* UX FIX: Disclaimer moved up to set expectations (Point 5) */}
...
const GreenwashingView = memo(({ navigateTo }) => {
   ...
   return (
     <div className="w-full">
            ...
            {/* ... Other sections would be memoized components here for brevity ... */}
            {/* Simplified for the "File Length" constraint, assuming standard sections present */}
...
/* ==================================================================================
   ...
   Manages Routing (Point 2) and Global State.
   ================================================================================== */
...
const App = React.forwardRef((props, ref) => {
  ...
  return (
    <div className={`min-h-screen font-sans text-[${THEME.colors.text.primary}] bg-[${THEME.colors.background}] selection:bg-black selection:text-white overflow-x-hidden relative`}>
      ...
      {/* --- SIDEBAR NAV --- */}
      <nav className="fixed left-0 top-0 h-full w-64 hidden lg:flex flex-col justify-between items-start py-12 pl-10 z-50 pointer-events-none" aria-label="Main Navigation">
         ...
         <div className="flex flex-col gap-6 pointer-events-auto w-full">
            {navItems.filter(item => !item.hidden).map((item) => ( 
                <button key={item.id} onClick={() => navigateTo(item.id)} className="group flex items-center gap-4 w-full text-left focus:outline-none" aria-current={activeSection === item.id ? 'page' : undefined}>
      ...
      {/* --- MOBILE HEADER --- */}
      <header className="fixed top-0 left-0 right-0 z-50 lg:hidden bg-[#E0E5EC]/90 backdrop-blur-md shadow-sm border-b border-white/50">
          <div className="h-20 flex items-center justify-between px-6">
            ...
            <button onClick={() => setIsMenuOpen(true)} className="p-3 rounded-xl bg-[#E0E5EC] shadow-[5px_5px_10px_#bebebe,-5px_-5px_10px_#ffffff] text-black" aria-label="Open menu">
              <Menu size={24} />
            </button>
          </div>
      </header>
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 z-[60] bg-[#E0E5EC] flex flex-col items-center justify-center space-y-8 animate-fade-in lg:hidden">
          <button onClick={() => setIsMenuOpen(false)} className="absolute top-6 right-6 p-4 rounded-full bg-[#E0E5EC] shadow-[5px_5px_10px_#bebebe,-5px_-5px_10px_#ffffff] text-black" aria-label="Close menu">
            <X size={24} />
          </button>
          {navItems.filter(item => !item.hidden).map((item) => (
            <button key={item.id} onClick={() => navigateTo(item.id)} className={`text-2xl font-bold tracking-tight ${activeSection === item.id ? 'text-black' : 'text-gray-500'}`}>
              {item.label}
            </button>
          ))}
        </div>
